...
The `EspLogger` class provides comprehensive error reporting through the `hf_logger_err_t` enumeration:
...
The `EspLogger` class uses mutex protection for thread-safe operation. Multiple threads can safely call logging methods simultaneously.
