/**
 * @file DigitalOutputGuardTest.cpp * @brief Simple test to verify DigitalOutputGuard functionality with unified BaseGpio
 * 
...
public:
    MockDigitalGpio(hf_gpio_num_t pin_num, Direction direction = Direction::Input)
        : BaseGpio(pin_num, direction), init_called_(false), 
          set_direction_called_(false), set_active_called_(false), 
...
public:
    ...
    bool WasSetDirectionCalled() const { return set_direction_called_; }
    bool WasSetActiveCalled() const { return set_active_called_; }
    bool WasSetInactiveCalled() const { return set_inactive_called_; }
...
int main() {
    ...
    try {
        ...
        std::cout << "DigitalOutputGuard is properly integrated with DigitalGpio interface." << std::endl;
