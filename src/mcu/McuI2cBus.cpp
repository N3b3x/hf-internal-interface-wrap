...
#include "../../include/mcu/McuI2cBus.h"
...
static const char* TAG = "McuI2cBus";
#endif
...
McuI2cBus::McuI2cBus(const I2cBusConfig& config) noexcept
    : BaseI2cBus(config),
...
bool McuI2cBus::Initialize() noexcept {
    if (initialized_) {
        return true;
    }
...
bool McuI2cBus::Deinitialize() noexcept {
    if (!initialized_) {
        return true;
    }
...
HfI2cErr McuI2cBus::Write(uint8_t device_addr, const uint8_t* data,
...
HfI2cErr McuI2cBus::Read(uint8_t device_addr, uint8_t* data,
                         uint16_t length, uint32_t timeout_ms) noexcept {
    if (!EnsureInitialized()) {
        return HfI2cErr::I2C_ERR_NOT_INITIALIZED;
    }
...
HfI2cErr McuI2cBus::WriteRead(uint8_t device_addr, const uint8_t* tx_data, uint16_t tx_length,
...
bool McuI2cBus::IsBusy() noexcept {
    if (!initialized_) {
        return false;
    }
...
bool McuI2cBus::ResetBus() noexcept {
    if (!initialized_) {
        return false;
    }
...
bool McuI2cBus::SetClockSpeed(uint32_t clock_speed_hz) noexcept {
...
bool McuI2cBus::SetPullUps(bool enable) noexcept {
...
uint32_t McuI2cBus::GetBusStatus() noexcept {
#ifdef MCU_PLATFORM_ESP32
...
bool McuI2cBus::RecoverBus() noexcept {
...
HfI2cErr McuI2cBus::ConvertPlatformError(int32_t platform_error) noexcept {
#ifdef MCU_PLATFORM_ESP32
...
bool McuI2cBus::PlatformInitialize() noexcept {
#ifdef MCU_PLATFORM_ESP32
...
bool McuI2cBus::PlatformDeinitialize() noexcept {
#ifdef MCU_PLATFORM_ESP32
