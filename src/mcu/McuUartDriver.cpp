/**
 * @file McuUartDriver.cpp
 * @brief Implementation of MCU-integrated UART controller.
 */

#include "../../include/mcu/McuUartDriver.h"
#include <cstring>
#include <cstdio>
...
static const char* TAG = "McuUartDriver";
#endif
...
McuUartDriver::McuUartDriver(hf_uart_port_t port, const UartConfig& config) noexcept
    : BaseUartDriver(port, config),
...
McuUartDriver::~McuUartDriver() noexcept {
    if (initialized_) {
        Deinitialize();
    }
}
...
bool McuUartDriver::Initialize() noexcept {
    if (initialized_) {
        return true;
    }
...
bool McuUartDriver::Deinitialize() noexcept {
    if (!initialized_) {
        return true;
    }
...
HfUartErr McuUartDriver::Write(const uint8_t* data, uint16_t length,
...
HfUartErr McuUartDriver::Read(uint8_t* data, uint16_t length,
                              uint32_t timeout_ms) noexcept {
    if (!EnsureInitialized()) {
        return HfUartErr::UART_ERR_NOT_INITIALIZED;
    }
...
uint16_t McuUartDriver::BytesAvailable() noexcept {
    if (!EnsureInitialized()) {
        return 0;
    }
...
HfUartErr McuUartDriver::FlushTx() noexcept {
    if (!EnsureInitialized()) {
        return HfUartErr::UART_ERR_NOT_INITIALIZED;
    }
...
HfUartErr McuUartDriver::FlushRx() noexcept {
    if (!EnsureInitialized()) {
        return HfUartErr::UART_ERR_NOT_INITIALIZED;
    }
...
int McuUartDriver::Printf(const char* format, ...) noexcept {
...
bool McuUartDriver::IsTxBusy() noexcept {
    return tx_in_progress_;
}

bool McuUartDriver::SetBaudRate(uint32_t baud_rate) noexcept {
...
bool McuUartDriver::SetFlowControl(bool enable) noexcept {
...
uint32_t McuUartDriver::GetUartStatus() noexcept {
#ifdef MCU_PLATFORM_ESP32
...
bool McuUartDriver::SetRTS(bool active) noexcept {
...
bool McuUartDriver::GetCTS() noexcept {
...
bool McuUartDriver::SendBreak(uint32_t duration_ms) noexcept {
    if (!EnsureInitialized()) {
        return false;
    }
...
bool McuUartDriver::IsBreakDetected() noexcept {
    bool detected = break_detected_;
    break_detected_ = false;  // Clear flag after reading
    return detected;
}

uint16_t McuUartDriver::TxBytesWaiting() noexcept {
    if (!EnsureInitialized()) {
        return 0;
    }
...
bool McuUartDriver::SetLoopback(bool enable) noexcept {
    if (!EnsureInitialized()) {
        return false;
    }
...
bool McuUartDriver::WaitTransmitComplete(uint32_t timeout_ms) noexcept {
    if (!EnsureInitialized()) {
        return false;
    }
...
uint16_t McuUartDriver::ReadUntil(uint8_t* data, uint16_t max_length, 
...
uint16_t McuUartDriver::ReadLine(char* buffer, uint16_t max_length, uint32_t timeout_ms) noexcept {
...
HfUartErr McuUartDriver::ConvertPlatformError(int32_t platform_error) noexcept {
#ifdef MCU_PLATFORM_ESP32
...
bool McuUartDriver::PlatformInitialize() noexcept {
#ifdef MCU_PLATFORM_ESP32
...
bool McuUartDriver::PlatformDeinitialize() noexcept {
#ifdef MCU_PLATFORM_ESP32
...
int McuUartDriver::InternalPrintf(const char* format, va_list args) noexcept {
