/**
 * @file EspI2c.cpp
 * @brief Complete implementation of ESP32-integrated I2C controller for ESP-IDF v5.5+ with ESP32C6
 * support.
 *
...
EspI2c::EspI2c(const hf_i2c_master_bus_config_t& config) noexcept
    : BaseI2c(), bus_config_(config), master_bus_handle_(nullptr), initialized_(false),
      bus_suspended_(false), current_power_mode_(hf_i2c_power_mode_t::HF_I2C_POWER_MODE_LOW),
      last_error_(hf_i2c_err_t::I2C_SUCCESS), last_operation_time_us_(0), statistics_{},
      diagnostics_{}, event_callback_(nullptr), event_user_data_(nullptr), mutex_(),
      stats_mutex_() {}
...
bool EspI2c::Initialize() noexcept {
  ...
  if (bus_config_.sda_io_num == static_cast<hf_pin_num_t>(HF_INVALID_PIN) ||
    ...
    ESP_LOGE(TAG, "Invalid GPIO pins: SDA=%d, SCL=%d", static_cast<int>(bus_config_.sda_io_num),
  ...
  ESP_LOGI(TAG, "I2C master bus initialized successfully on port %d",
           static_cast<int>(bus_config_.i2c_port));
...
hf_i2c_err_t EspI2c::Write(hf_u8_t device_addr, const hf_u8_t* data, hf_u16_t length,
  ...
  esp_err_t ret =
      i2c_master_transmit(dev_handle, data, length, static_cast<int>(effective_timeout));
  ...
  if (result == hf_i2c_err_t::I2C_SUCCESS) {
    ...
    ESP_LOGW(TAG, "I2C write failed: addr=0x%02X, len=%d, error=%s", device_addr, length,
             esp_err_to_name(ret));
  }
...
hf_i2c_err_t EspI2c::Read(hf_u8_t device_addr, hf_u8_t* data, hf_u16_t length,
  ...
  hf_u32_t effective_timeout = GetEffectiveTimeout(timeout_ms);
  esp_err_t ret = i2c_master_receive(dev_handle, data, length, static_cast<int>(effective_timeout));
  ...
  if (result == hf_i2c_err_t::I2C_SUCCESS) {
    ...
    ESP_LOGW(TAG, "I2C read failed: addr=0x%02X, len=%d, error=%s", device_addr, length,
             esp_err_to_name(ret));
  }
...
hf_i2c_err_t EspI2c::WriteRead(hf_u8_t device_addr, const hf_u8_t* tx_data, hf_u16_t tx_length,
  ...
  hf_u32_t effective_timeout = GetEffectiveTimeout(timeout_ms);
  esp_err_t ret = i2c_master_transmit_receive(dev_handle, tx_data, tx_length, rx_data, rx_length,
                                              static_cast<int>(effective_timeout));

  result = ConvertEspError(ret);

  if (result == hf_i2c_err_t::I2C_SUCCESS) {
    ESP_LOGD(TAG, "I2C write-read successful: addr=0x%02X, tx_len=%d, rx_len=%d", device_addr,
             tx_length, rx_length);
  } else {
    ESP_LOGW(TAG, "I2C write-read failed: addr=0x%02X, tx_len=%d, rx_len=%d, error=%s", device_addr,
             tx_length, rx_length, esp_err_to_name(ret));
  }
...
hf_i2c_err_t EspI2c::AddDevice(const hf_i2c_device_config_t& device_config) noexcept {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Failed to add device 0x%02X: %s", device_config.device_address,
             esp_err_to_name(ret));
  ...
  if (event_callback_) {
    event_callback_(hf_i2c_event_type_t::HF_I2C_EVENT_SLAVE_READ,
                    const_cast<hf_i2c_device_config_t*>(&device_config), event_user_data_);
  }
...
hf_i2c_err_t EspI2c::RemoveDevice(hf_u16_t device_address) noexcept {
  ...
  // Trigger event callback if registered
  if (event_callback_) {
    event_callback_(hf_i2c_event_type_t::HF_I2C_EVENT_SLAVE_READ, &device_address,
                    event_user_data_);
  }
 ...
 */
void EspI2c::UpdateStatistics(bool success, size_t bytes_transferred,
                              uint64_t operation_time_us) noexcept {
  ...
  if (total_ops > 0) {
    ...
    if (now - last_activity < 1000000) { // Active within last second
      diagnostics_.bus_utilization_percent =
          std::min(100.0f, static_cast<float>(total_ops) / 1000.0f * 100.0f);
    } else {
 ...
 */
size_t EspI2c::ScanDevices(std::vector<hf_u16_t>& found_devices, hf_u16_t start_addr,
...
hf_i2c_err_t EspI2c::ReadMultipleRegisters(hf_u16_t device_addr, hf_u8_t start_reg_addr,
  ...
  data.resize(count);
  return WriteRead(static_cast<uint8_t>(device_addr), &start_reg_addr, 1, data.data(),
...
hf_i2c_err_t EspI2c::WriteAsync(hf_u16_t device_addr, const std::vector<hf_u8_t>& data,
  ...
  hf_i2c_err_t result =
      Write(static_cast<uint8_t>(device_addr), data.data(), static_cast<uint16_t>(data.size()));
 ...
 * @return Operation result with operation ID for tracking
 */
hf_i2c_err_t EspI2c::ReadAsync(hf_u16_t device_addr, size_t length,
                               hf_i2c_async_callback_t callback, void* user_data) noexcept {
  if (!EnsureInitialized()) {
    return hf_i2c_err_t::I2C_ERR_NOT_INITIALIZED;
  }
  ...
  // For now, perform synchronous operation and call callback immediately
  hf_i2c_err_t result =
      Read(static_cast<uint8_t>(device_addr), buffer.data(), static_cast<uint16_t>(length));
 ...
 * @return Operation result
 */
hf_i2c_err_t EspI2c::ExecuteMultiBufferTransaction(
    const hf_i2c_multi_buffer_transaction_t& transaction) noexcept {
  if (!EnsureInitialized()) {
    return hf_i2c_err_t::I2C_ERR_NOT_INITIALIZED;
  }
 ...
 * @return Operation result
 */
hf_i2c_err_t EspI2c::ExecuteMultiBufferTransactionAsync(
    const hf_i2c_multi_buffer_transaction_t& transaction, hf_i2c_async_callback_t callback,
 ...
 * @return Operation result
 */
hf_i2c_err_t EspI2c::ExecuteCustomSequence(
    const std::vector<hf_i2c_custom_command_t>& commands) noexcept {
  if (!EnsureInitialized()) {
    return hf_i2c_err_t::I2C_ERR_NOT_INITIALIZED;
  }
  ...
  for (const auto& command : commands) {
    ...
    switch (command.command_type) {
      case hf_i2c_command_type_t::HF_I2C_CMD_WRITE:
        result = Write(static_cast<uint8_t>(command.device_addr), command.data.data(),
                       static_cast<uint16_t>(command.data.size()), command.timeout_ms);
        break;

      case hf_i2c_command_type_t::HF_I2C_CMD_READ: {
        std::vector<uint8_t> read_buffer(command.expected_read_size);
        result = Read(static_cast<uint8_t>(command.device_addr), read_buffer.data(),
                      static_cast<uint16_t>(read_buffer.size()), command.timeout_ms);
      } break;

      case hf_i2c_command_type_t::HF_I2C_CMD_WRITE_READ: {
        std::vector<uint8_t> read_buffer(command.expected_read_size);
        result = WriteRead(static_cast<uint8_t>(command.device_addr), command.data.data(),
                           static_cast<uint16_t>(command.data.size()), read_buffer.data(),
                           static_cast<uint16_t>(read_buffer.size()), command.timeout_ms);
      } break;
    ...
    if (result != hf_i2c_err_t::I2C_SUCCESS) {
      ESP_LOGW(TAG, "Custom sequence failed at command %zu", &command - &commands[0]);
      return result;
    }
  }
 ...
 * @return Operation result
 */
hf_i2c_err_t EspI2c::ExecuteCustomSequenceAsync(
    const std::vector<hf_i2c_custom_command_t>& commands, hf_i2c_async_callback_t callback,
    void* user_data) noexcept {
  if (!callback) {
    return hf_i2c_err_t::I2C_ERR_INVALID_PARAMETER;
  }
