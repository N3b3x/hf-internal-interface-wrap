...
EspSpiDevice::EspSpiDevice(EspSpiBus* parent, spi_device_handle_t handle, const hf_spi_device_config_t& config)
  : BaseSpi(), parent_bus_(parent), handle_(handle), config_(config), initialized_(true) {}  // Properly initialize BaseSpi
...
hf_spi_err_t EspSpiDevice::Transfer(const hf_u8_t* tx_data, hf_u8_t* rx_data, hf_u16_t length, hf_u32_t timeout_ms) noexcept {
  ...
  if (length > HF_SPI_MAX_TRANSFER_SIZE) return hf_spi_err_t::SPI_ERR_TRANSFER_TOO_LONG;
  if (!tx_data && !rx_data) return hf_spi_err_t::SPI_ERR_NULL_POINTER;
