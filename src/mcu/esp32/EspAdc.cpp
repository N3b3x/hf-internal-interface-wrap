 ...
 */
[[maybe_unused]] static esp_err_t GpioToAdcChannel(int gpio_num, adc_unit_t unit_id, adc_channel_t* channel) {
 ...
 * @return ESP_OK on success, error code on failure
 */
[[maybe_unused]] static esp_err_t AdcChannelToGpio(adc_unit_t unit_id, adc_channel_t channel, int* gpio_num) {
    ...
    if (config_.calibration_config.enable_calibration) {
      for (const auto& channel_config : config_.channel_configs) {
        if (channel_config.enabled) {
          ...
          esp_err = adc_cali_create_scheme_curve_fitting(
              &cali_config,
...
hf_adc_err_t EspAdc::ConfigureChannel(hf_channel_id_t channel_id, hf_adc_atten_t attenuation,
  ...
  if (result == hf_adc_err_t::ADC_SUCCESS) {
    ...
    if (oneshot_handle_ != nullptr && channel_config.enabled) {
      ...
      esp_err_t esp_err = adc_oneshot_config_channel(
          oneshot_handle_, static_cast<adc_channel_t>(channel_id), &chan_cfg);
...
hf_adc_err_t EspAdc::EnableChannel(hf_channel_id_t channel_id) noexcept {
  ...
  if (!config_.channel_configs[channel_id].enabled) {
    ...
    if (oneshot_handle_ != nullptr) {
      return ConfigureChannel(channel_id, config_.channel_configs[channel_id].attenuation,
...
hf_adc_err_t EspAdc::ReadSingleVoltage(hf_channel_id_t channel_id, hf_u32_t& voltage_mv) noexcept {
  ...
  if (result == hf_adc_err_t::ADC_SUCCESS) {
    ...
    if (calibration_handles_[atten] != nullptr) {
      ...
      const hf_u32_t max_voltage_mv =
          (atten == static_cast<hf_u8_t>(hf_adc_atten_t::ATTEN_DB_0))     ? 950
          : (atten == static_cast<hf_u8_t>(hf_adc_atten_t::ATTEN_DB_2_5)) ? 1250
          : (atten == static_cast<hf_u8_t>(hf_adc_atten_t::ATTEN_DB_6))   ? 1750
                                                                          : 2450;
...
hf_adc_err_t EspAdc::ReadMultipleRaw(const hf_channel_id_t* channel_ids, hf_u8_t num_channels,
...
hf_adc_err_t EspAdc::ReadMultipleVoltage(const hf_channel_id_t* channel_ids, hf_u8_t num_channels,
...
hf_adc_err_t EspAdc::ConfigureContinuous(const hf_adc_continuous_config_t& config) noexcept {
  ...
  if (!IsValidContinuousConfig(config.samples_per_frame, enabled_count, config.max_store_frames)) {
    ESP_LOGE(TAG,
             "Invalid continuous configuration: samples_per_frame=%lu, enabled_channels=%lu, "
             "max_store_frames=%lu",
...
hf_adc_err_t EspAdc::ReadContinuousData(hf_u8_t* buffer, hf_u32_t buffer_size, hf_u32_t& bytes_read,
  ...
  hf_u32_t ret_num = 0;
  esp_err_t esp_err =
      adc_continuous_read(continuous_handle_, buffer, buffer_size, &ret_num, timeout_ms);
...
hf_adc_err_t EspAdc::InitializeCalibration(hf_adc_atten_t attenuation,
  ...
  esp_err_t esp_err =
      adc_cali_create_scheme_curve_fitting(&cali_config, &calibration_handles_[atten_idx]);
...
hf_adc_err_t EspAdc::RawToVoltage(hf_u32_t raw_count, hf_adc_atten_t attenuation,
...
hf_adc_err_t EspAdc::ReadChannel(hf_channel_id_t channel_id, hf_u32_t& channel_reading_count,
  ...
  hf_adc_err_t result =
      ReadChannelCount(channel_id, channel_reading_count, numOfSamplesToAvg, timeBetweenSamples);
  ...
  if (result == hf_adc_err_t::ADC_SUCCESS) {
    ...
    if (calibration_handles_[atten] != nullptr) {
      int voltage_cal;
      esp_err_t esp_err = adc_cali_raw_to_voltage(
          calibration_handles_[atten], static_cast<int>(channel_reading_count), &voltage_cal);
...
hf_bool_t IRAM_ATTR EspAdc::ContinuousCallback(adc_continuous_handle_t handle,
  ...
  // ESP32 uses TYPE2 format: 12-bit data in 32-bit structure (4 bytes per sample)
...
hf_adc_err_t EspAdc::InitializeOneshot() noexcept {
  ...
  for (hf_u8_t i = 0; i < HF_ESP32_ADC_MAX_CHANNELS; i++) {
    if (config_.channel_configs[i].enabled) {
      adc_oneshot_chan_cfg_t chan_cfg = {
          .atten = static_cast<adc_atten_t>(
              static_cast<hf_u8_t>(config_.channel_configs[i].attenuation)),
          .bitwidth = static_cast<adc_bitwidth_t>(
              static_cast<hf_u8_t>(config_.channel_configs[i].bitwidth))};

      esp_result =
          adc_oneshot_config_channel(oneshot_handle_, static_cast<adc_channel_t>(i), &chan_cfg);
      if (esp_result != ESP_OK) {
...
hf_adc_err_t EspAdc::InitializeContinuous() noexcept {
  ...
  for (hf_u8_t i = 0; i < HF_ESP32_ADC_MAX_CHANNELS; i++) {
    if (config_.channel_configs[i].enabled) {
      adc_digi_pattern_config_t pattern = {
          ...
          .bit_width = static_cast<adc_bitwidth_t>(
              static_cast<hf_u8_t>(config_.channel_configs[i].bitwidth))};
  ...
  hf_u32_t buffer_pool_size =
      CalcBufferPoolSize(config_.continuous_config.samples_per_frame, enabled_count,
                         config_.continuous_config.max_store_frames);

  adc_continuous_handle_cfg_t adc_config = {
      .max_store_buf_size = buffer_pool_size,
      .conv_frame_size = frame_size,
      .flags = {.flush_pool = config_.continuous_config.flush_pool}};

  ESP_LOGI(
      TAG,
      "Continuous ADC config: samples_per_frame=%lu, enabled_channels=%u, max_store_frames=%lu",
      config_.continuous_config.samples_per_frame, enabled_count,
      config_.continuous_config.max_store_frames);
  ESP_LOGI(TAG, "Calculated: frame_size=%lu bytes, buffer_pool_size=%lu bytes", frame_size,
           buffer_pool_size);
  ...
  adc_continuous_config_t dig_cfg = {
      ...
      .format = ADC_DIGI_OUTPUT_FORMAT_TYPE2, // ESP32 requires TYPE2
  };
...
hf_adc_err_t EspAdc::ReadOneshotRaw(hf_channel_id_t channel_id, hf_u32_t& raw_value) noexcept {
  ...
  esp_err_t esp_result =
      adc_oneshot_read(oneshot_handle_, static_cast<adc_channel_t>(channel_id), &raw_reading);
...
hf_adc_err_t EspAdc::ValidateConfiguration() const noexcept {
  ...
  if (config_.unit_id >= HF_ESP32_ADC_MAX_UNITS) {
    ESP_LOGE(TAG, "Invalid ADC unit %d (this ESP32 variant supports %d units)", config_.unit_id,
  ...
  if (config_.mode == hf_adc_mode_t::CONTINUOUS) {
    ...
    // Validate user-friendly parameters using utility functions
    if (!IsValidContinuousConfig(config_.continuous_config.samples_per_frame, enabled_count,
                                 config_.continuous_config.max_store_frames)) {
      ESP_LOGE(TAG,
               "Invalid continuous configuration: samples_per_frame=%lu, enabled_channels=%lu, "
               "max_store_frames=%lu",
  ...
  if (result == hf_adc_err_t::ADC_SUCCESS) {
    ...
    if (statistics_.successfulConversions == 1) {
      ...
      statistics_.averageConversionTimeUs =
          (statistics_.averageConversionTimeUs * (statistics_.successfulConversions - 1) +
           conversion_time_us) /
          statistics_.successfulConversions;
    }
  } else {
...
void EspAdc::UpdateDiagnostics(hf_adc_err_t error) noexcept {
  ...
  if (error != hf_adc_err_t::ADC_SUCCESS) {
    ...
    diagnostics_.adcHealthy =
        (diagnostics_.consecutiveErrors < 10); // Mark unhealthy after 10 consecutive errors
  } else {
