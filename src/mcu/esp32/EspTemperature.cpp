...
EspTemperature::EspTemperature() noexcept
    : BaseTemperature(), mutex_(), esp_state_{}, esp_config_(ESP_TEMP_CONFIG_DEFAULT()),
      base_config_(HF_TEMP_CONFIG_DEFAULT()), statistics_{}, diagnostics_{},
      last_error_(hf_temp_err_t::TEMP_SUCCESS), threshold_callback_(nullptr),
      monitoring_callback_(nullptr), esp_threshold_callback_(nullptr),
      esp_monitoring_callback_(nullptr), threshold_user_data_(nullptr),
...
EspTemperature::EspTemperature(const esp_temp_config_t& esp_config) noexcept : EspTemperature() {
  esp_config_ = esp_config;
  ESP_LOGD(TAG, "EspTemperature instance created with custom configuration");
}

EspTemperature::~EspTemperature() noexcept {
  // Avoid locking here to prevent nested lock in Deinitialize()
  if (initialized_) {
    Deinitialize();
  }
...
bool EspTemperature::Initialize() noexcept {
...

  {
    ...
    if (s_shared_handle_ == nullptr) {
      ...
      esp_err_t esp_err = temperature_sensor_install(&temp_config, &s_shared_handle_);
      if (esp_err != ESP_OK) {
      ...
      esp_err = temperature_sensor_enable(s_shared_handle_);
      if (esp_err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to enable temperature sensor: %s", esp_err_to_name(esp_err));
        temperature_sensor_uninstall(s_shared_handle_);
    ...
    s_refcount_++;
    esp_state_.handle = s_shared_handle_;
  }
  ...
  // Setup threshold monitoring bookkeeping only (avoid nested locking here)
...
bool EspTemperature::Deinitialize() noexcept {
  ...
  if (esp_state_.continuous_monitoring_active) {
    // Inline StopContinuousMonitoring() without taking the lock again
    bool success_monitor = true;
    ...
    monitoring_user_data_ = nullptr;
    (void)success_monitor;
  }

  // Disable threshold monitoring if active
  if (esp_state_.threshold_monitoring_enabled) {
    // Inline DisableThresholdMonitoring() without taking the lock again
    esp_state_.threshold_monitoring_enabled = false;
    diagnostics_.threshold_monitoring_enabled = false;
  ...
  if (esp_state_.handle != nullptr) {
    ...
    if (s_refcount_ == 0 && s_shared_handle_ != nullptr) {
      esp_err_t esp_err = temperature_sensor_disable(s_shared_handle_);
      if (esp_err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to disable temperature sensor: %s", esp_err_to_name(esp_err));
        success = false;
      }
      esp_err = temperature_sensor_uninstall(s_shared_handle_);
      if (esp_err != ESP_OK) {
...
hf_temp_err_t EspTemperature::ReadTemperatureCelsiusImpl(float* temperature_celsius) noexcept {
  ...
  if (!initialized_) {
    ...
    return SetLastError(hf_temp_err_t::TEMP_ERR_NOT_INITIALIZED),
           hf_temp_err_t::TEMP_ERR_NOT_INITIALIZED;
  }

  if (esp_state_.handle == nullptr) {
    ESP_LOGE(TAG, "ESP sensor handle is null");
    UpdateStatistics(false, GetCurrentTimeUs() - start_time);
    return SetLastError(hf_temp_err_t::TEMP_ERR_SENSOR_NOT_AVAILABLE),
           hf_temp_err_t::TEMP_ERR_SENSOR_NOT_AVAILABLE;
  }
  ...
  *temperature_celsius = raw_temp + esp_state_.calibration_offset;
  ...
  if (statistics_.temperature_readings == 1) {
    ...
    statistics_.avg_temperature_celsius =
        (statistics_.avg_temperature_celsius * (statistics_.temperature_readings - 1) +
         *temperature_celsius) /
        statistics_.temperature_readings;
  }
  ...
  ESP_LOGD(TAG, "Temperature reading: %.2f°C (raw: %.2f°C, offset: %.2f°C)", *temperature_celsius,
           raw_temp, esp_state_.calibration_offset);
...
hf_u32_t EspTemperature::GetCapabilities() const noexcept {
  return HF_TEMP_CAP_THRESHOLD_MONITORING | HF_TEMP_CAP_CONTINUOUS_READING |
         HF_TEMP_CAP_CALIBRATION | HF_TEMP_CAP_POWER_MANAGEMENT | HF_TEMP_CAP_SELF_TEST |
...
hf_temp_err_t EspTemperature::SetRange(float min_celsius, float max_celsius) noexcept {
  if (min_celsius >= max_celsius) {
    ...
    return SetLastError(hf_temp_err_t::TEMP_ERR_INVALID_RANGE),
           hf_temp_err_t::TEMP_ERR_INVALID_RANGE;
  }

  // Find optimal range for the given requirements (no lock needed)
  ...
  if (optimal_range >= ESP_TEMP_RANGE_COUNT) {
    ...
    return SetLastError(hf_temp_err_t::TEMP_ERR_UNSUPPORTED_RANGE),
           hf_temp_err_t::TEMP_ERR_UNSUPPORTED_RANGE;
  }

  // Apply range change with internal locking
...
hf_temp_err_t EspTemperature::SetThresholds(float low_threshold_celsius,
  ...
  if (low_threshold_celsius >= high_threshold_celsius) {
    ...
    return SetLastError(hf_temp_err_t::TEMP_ERR_INVALID_THRESHOLD),
           hf_temp_err_t::TEMP_ERR_INVALID_THRESHOLD;
  }
  ...
  ESP_LOGI(TAG, "Temperature thresholds set: Low=%.1f°C, High=%.1f°C", low_threshold_celsius,
           high_threshold_celsius);
...
hf_temp_err_t EspTemperature::StartContinuousMonitoring(hf_u32_t sample_rate_hz,
  ...
  if (esp_state_.continuous_monitoring_active) {
    ...
    return SetLastError(hf_temp_err_t::TEMP_ERR_INVALID_STATE),
           hf_temp_err_t::TEMP_ERR_INVALID_STATE;
  }

  if (sample_rate_hz < ESP_TEMP_MIN_SAMPLE_RATE_HZ ||
      sample_rate_hz > ESP_TEMP_MAX_SAMPLE_RATE_HZ) {
    ESP_LOGE(TAG, "Invalid sample rate: %u Hz (valid range: %d-%d Hz)", sample_rate_hz,
             ESP_TEMP_MIN_SAMPLE_RATE_HZ, ESP_TEMP_MAX_SAMPLE_RATE_HZ);
    return SetLastError(hf_temp_err_t::TEMP_ERR_INVALID_PARAMETER),
           hf_temp_err_t::TEMP_ERR_INVALID_PARAMETER;
  }
  ...
  const esp_timer_create_args_t timer_args = {.callback = MonitoringTimerCallback,
                                              .arg = this,
...
hf_temp_err_t EspTemperature::EnterSleepMode() noexcept {
  ...
  if (!esp_state_.allow_power_down) {
    ...
    return SetLastError(hf_temp_err_t::TEMP_ERR_UNSUPPORTED_OPERATION),
           hf_temp_err_t::TEMP_ERR_UNSUPPORTED_OPERATION;
  }
...
hf_temp_err_t EspTemperature::SelfTest() noexcept {
  ...
  if (error != hf_temp_err_t::TEMP_SUCCESS) {
    ...
    return SetLastError(hf_temp_err_t::TEMP_ERR_HARDWARE_FAULT),
           hf_temp_err_t::TEMP_ERR_HARDWARE_FAULT;
  }
  ...
  if (temperature < -50.0f || temperature > 150.0f) {
    ...
    return SetLastError(hf_temp_err_t::TEMP_ERR_INVALID_READING),
           hf_temp_err_t::TEMP_ERR_INVALID_READING;
  }
  ...
  ESP_LOGI(TAG, "Self-test passed: Temperature=%.2f°C, Variation=%.2f°C", temperature,
           max_temp - min_temp);
...
hf_temp_err_t EspTemperature::GetDiagnostics(hf_temp_diagnostics_t& diagnostics) noexcept {
  ...
  diagnostics_.current_temperature_raw =
      static_cast<hf_u32_t>(esp_state_.last_temperature_celsius * 1000);
...
hf_temp_err_t EspTemperature::SetMeasurementRange(esp_temp_range_t range) noexcept {
  ...
  if (initialized_ && esp_state_.handle != nullptr) {
    ...
    ESP_LOGI(TAG, "Range changed to %d: %.0f°C to %.0f°C (±%.1f°C)", static_cast<int>(range),
...
void EspTemperature::GetRangeConfig(esp_temp_range_t range, float* min_celsius, float* max_celsius,
                                    float* accuracy_celsius) const noexcept {
  if (range < ESP_TEMP_RANGE_COUNT) {
    const esp_temp_range_info_t& info = RANGE_INFO[range];
    if (min_celsius)
      *min_celsius = info.min_celsius;
    if (max_celsius)
      *max_celsius = info.max_celsius;
    if (accuracy_celsius)
      *accuracy_celsius = info.accuracy_celsius;
  } else {
    // Default to first range for invalid input
    if (min_celsius)
      *min_celsius = RANGE_INFO[0].min_celsius;
    if (max_celsius)
      *max_celsius = RANGE_INFO[0].max_celsius;
    if (accuracy_celsius)
      *accuracy_celsius = RANGE_INFO[0].accuracy_celsius;
  }
}
...
void EspTemperature::UpdateStatistics(bool operation_successful,
                                      hf_u32_t operation_time_us) noexcept {
  ...
  if (statistics_.total_operations == 1) {
    ...
    statistics_.average_operation_time_us =
        (statistics_.average_operation_time_us * (statistics_.total_operations - 1) +
         operation_time_us) /
        statistics_.total_operations;
  }
}
...
void EspTemperature::UpdateDiagnostics(hf_temp_err_t error) noexcept {
  if (error != hf_temp_err_t::TEMP_SUCCESS) {
    ...
    diagnostics_.last_error_timestamp =
        static_cast<hf_u32_t>(GetCurrentTimeUs() / 1000); // Convert to ms
    diagnostics_.consecutive_errors++;
...
void EspTemperature::CheckThresholds(float temperature) noexcept {
  ...
  if (temperature <= esp_config_.low_threshold_celsius) {
    ...
    ESP_LOGW(TAG, "Low threshold violated: %.2f°C <= %.2f°C", temperature,
             esp_config_.low_threshold_celsius);
  } else if (temperature >= esp_config_.high_threshold_celsius) {
    threshold_violated = true;
    is_high_threshold = true;
    ESP_LOGW(TAG, "High threshold violated: %.2f°C >= %.2f°C", temperature,
