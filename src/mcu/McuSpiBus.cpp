...
#include "../../include/mcu/McuSpiBus.h"
...
static const char* TAG = "McuSpiBus";
#endif
...
McuSpiBus::McuSpiBus(const SpiBusConfig& config) noexcept
    : BaseSpiBus(config),
...
bool McuSpiBus::Initialize() noexcept {
    if (initialized_) {
        return true;
    }
...
bool McuSpiBus::Deinitialize() noexcept {
    if (!initialized_) {
        return true;
    }
...
HfSpiErr McuSpiBus::Transfer(const uint8_t* tx_data, uint8_t* rx_data,
...
HfSpiErr McuSpiBus::SetChipSelect(bool active) noexcept {
    if (!EnsureInitialized()) {
        return HfSpiErr::SPI_ERR_NOT_INITIALIZED;
    }
...
bool McuSpiBus::IsBusy() noexcept {
    if (!initialized_) {
        return false;
    }
...
bool McuSpiBus::SetClockSpeed(uint32_t clock_speed_hz) noexcept {
...
bool McuSpiBus::SetMode(uint8_t mode) noexcept {
    if (!IsValidMode(mode)) {
        return false;
    }
...
bool McuSpiBus::SetDmaEnabled(bool enable) noexcept {
    dma_enabled_ = enable;
    return true;  // DMA setting will be applied on next initialization
}

uint32_t McuSpiBus::GetBusStatus() noexcept {
#ifdef MCU_PLATFORM_ESP32
...
HfSpiErr McuSpiBus::TransferSequence(const SpiTransfer* transfers, uint8_t num_transfers) noexcept {
    if (!EnsureInitialized()) {
        return HfSpiErr::SPI_ERR_NOT_INITIALIZED;
    }
...
HfSpiErr McuSpiBus::TransferWithTiming(const uint8_t* tx_data, uint8_t* rx_data,
...
HfSpiErr McuSpiBus::WriteRegister(uint8_t reg_addr, const uint8_t* data, uint16_t length) noexcept {
    if (!data || length == 0) {
        return HfSpiErr::SPI_ERR_INVALID_PARAMETER;
    }
...
HfSpiErr McuSpiBus::ReadRegister(uint8_t reg_addr, uint8_t* data, uint16_t length) noexcept {
    if (!data || length == 0) {
        return HfSpiErr::SPI_ERR_INVALID_PARAMETER;
    }
...
HfSpiErr McuSpiBus::ConvertPlatformError(int32_t platform_error) noexcept {
#ifdef MCU_PLATFORM_ESP32
...
bool McuSpiBus::PlatformInitialize() noexcept {
#ifdef MCU_PLATFORM_ESP32
    // Configure SPI bus
...
bool McuSpiBus::PlatformDeinitialize() noexcept {
#ifdef MCU_PLATFORM_ESP32
...
HfSpiErr McuSpiBus::InternalTransfer(const uint8_t* tx_data, uint8_t* rx_data,
