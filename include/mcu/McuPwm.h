...
public:
    ...
    HfPwmErr ConfigureChannel(uint8_t channel_id, const PwmChannelConfig& config) noexcept override;
    HfPwmErr EnableChannel(uint8_t channel_id) noexcept override;
    HfPwmErr DisableChannel(uint8_t channel_id) noexcept override;
    bool IsChannelEnabled(uint8_t channel_id) const noexcept override;
    
    //==============================================================================
    // PWM CONTROL (BasePwm Interface)
    //==============================================================================
    
    HfPwmErr SetDutyCycle(uint8_t channel_id, float duty_cycle) noexcept override;
    HfPwmErr SetDutyCycleRaw(uint8_t channel_id, uint32_t raw_value) noexcept override;
    HfPwmErr SetFrequency(uint8_t channel_id, uint32_t frequency_hz) noexcept override;
    HfPwmErr SetPhaseShift(uint8_t channel_id, float phase_shift_degrees) noexcept override;
    ...
    float GetDutyCycle(uint8_t channel_id) const noexcept override;
    uint32_t GetFrequency(uint8_t channel_id) const noexcept override;
    HfPwmErr GetChannelStatus(uint8_t channel_id, PwmChannelStatus& status) const noexcept override;
    HfPwmErr GetCapabilities(PwmCapabilities& capabilities) const noexcept override;
    HfPwmErr GetLastError(uint8_t channel_id) const noexcept override;
     ...
     */
    HfPwmErr SetHardwareFade(uint8_t channel_id, float target_duty_cycle, uint32_t fade_time_ms) noexcept;
     ...
     * @return PWM_SUCCESS on success, error code on failure
     */
    HfPwmErr StopHardwareFade(uint8_t channel_id) noexcept;
     ...
     */
    bool IsFadeActive(uint8_t channel_id) const noexcept;
     ...
     * @return PWM_SUCCESS on success, error code on failure
     */
    HfPwmErr SetIdleLevel(uint8_t channel_id, uint8_t idle_level) noexcept;
     ...
     */
    int8_t GetTimerAssignment(uint8_t channel_id) const noexcept;
     ...
     */
    HfPwmErr ForceTimerAssignment(uint8_t channel_id, uint8_t timer_id) noexcept;
...
private:
     ...
     * @return PWM_SUCCESS on success, error code on failure
     */
    HfPwmErr ConfigurePlatformChannel(uint8_t channel_id, const PwmChannelConfig& config, uint8_t timer_id) noexcept;
     ...
     * @return PWM_SUCCESS on success, error code on failure
     */
    HfPwmErr UpdatePlatformDuty(uint8_t channel_id, uint32_t raw_duty_value) noexcept;
     ...
     */
    void SetChannelError(uint8_t channel_id, HfPwmErr error) noexcept;
    
    /**
     * @brief Platform-specific interrupt handler
     * @param channel_id Channel that generated interrupt
     */
    static void IRAM_ATTR InterruptHandler(uint8_t channel_id, void* user_data) noexcept;
    
    /**
     * @brief Handle fade complete interrupt
     * @param channel_id Channel that completed fade
     */
    void HandleFadeComplete(uint8_t channel_id) noexcept;
