 ...
 *       with appropriate synchronization guards if used within an ISR or shared
 *       between tasks.
 */
...
public:
     ...
     * @param channel_num Channel that completed conversion
     * @param samples Pointer to sample data
     * @param num_samples Number of samples in buffer
     * @param user_data User-provided data
     */
    using AdcCallback = std::function<void(uint8_t channel_num, const uint16_t* samples, 
     ...
     */
    virtual HfAdcErr ConfigureAdvanced(uint8_t channel_num, 
     ...
     * @note Default implementation returns unsupported operation
     */
    virtual HfAdcErr StartContinuousSampling(uint8_t channel_num,
     ...
     * @note Default implementation returns unsupported operation
     */
    virtual HfAdcErr StopContinuousSampling(uint8_t channel_num) noexcept {
        return HfAdcErr::ADC_ERR_UNSUPPORTED_OPERATION;
    }

    /**
     * @brief Read multiple channels simultaneously
     * @param channel_nums Array of channel numbers
     ...
     */
    virtual HfAdcErr ReadMultipleChannels(const uint8_t* channel_nums,
                                          uint8_t num_channels,
                                          uint32_t* readings,
                                          float* voltages) noexcept {
        if (!channel_nums || !readings || !voltages) {
            return HfAdcErr::ADC_ERR_NULL_POINTER;
        }

        for (uint8_t i = 0; i < num_channels; ++i) {
            HfAdcErr err = ReadChannel(channel_nums[i], readings[i], voltages[i]);
     ...
     * @param channel_num Channel being calibrated
     * @param progress_percent Progress percentage (0-100)
     * @param current_step Description of current calibration step
     * @param user_data User-provided data
     */
    using CalibrationProgressCallback = std::function<void(uint8_t channel_num, 
     ...
     * @note Default implementation returns unsupported operation
     */
    virtual HfAdcErr CalibrateChannel(uint8_t channel_num,
     ...
     */
    virtual HfAdcErr AutoCalibrate(uint8_t channel_num,
     ...
     * @param channel_num Channel calibration to save
     * @return HfAdcErr error code
     */
    virtual HfAdcErr SaveCalibration(uint8_t channel_num) noexcept {
        return HfAdcErr::ADC_ERR_UNSUPPORTED_OPERATION;
    }

    /**
     * @brief Load calibration data from non-volatile storage
     * @param channel_num Channel calibration to load
     * @return HfAdcErr error code
     */
    virtual HfAdcErr LoadCalibration(uint8_t channel_num) noexcept {
        return HfAdcErr::ADC_ERR_UNSUPPORTED_OPERATION;
    }

    /**
     * @brief Clear/reset calibration for a channel
     * @param channel_num Channel to reset
     * @return HfAdcErr error code
     */
    virtual HfAdcErr ClearCalibration(uint8_t channel_num) noexcept {
        return HfAdcErr::ADC_ERR_UNSUPPORTED_OPERATION;
    }

    /**
     * @brief Get calibration status and information
     * @param channel_num Channel to query
     * @param status Structure to fill with calibration status
     * @return HfAdcErr error code
     */
    virtual HfAdcErr GetCalibrationStatus(uint8_t channel_num, 
     ...
     * @return HfAdcErr error code
     */
    virtual HfAdcErr VerifyCalibration(uint8_t channel_num,
