...
#ifndef HAL_INTERNAL_INTERFACE_DRIVERS_BASEI2CBUS_H_
#define HAL_INTERNAL_INTERFACE_DRIVERS_BASEI2CBUS_H_
 ...
 *          supporting various MCU platforms and I2C modes.
 */
struct I2cBusConfig {
    hf_i2c_port_t port;                 ///< I2C port number
    hf_gpio_num_t sda_pin;              ///< SDA pin number
    hf_gpio_num_t scl_pin;              ///< SCL pin number
    uint32_t clock_speed_hz;            ///< Clock speed in Hz (typically 100kHz or 400kHz)
    ...
    I2cBusConfig() noexcept :
        port(0),
        sda_pin(HF_GPIO_INVALID),
        scl_pin(HF_GPIO_INVALID),
...
public:
     ...
     */
    explicit BaseI2cBus(const I2cBusConfig& config) noexcept :
        config_(config), initialized_(false) {}

    /**
     * @brief Virtual destructor ensures proper cleanup in derived classes.
     */
    virtual ~BaseI2cBus() noexcept = default;
    
    // Non-copyable, non-movable (can be changed in derived classes if needed)
    BaseI2cBus(const BaseI2cBus&) = delete;
    BaseI2cBus& operator=(const BaseI2cBus&) = delete;
    BaseI2cBus(BaseI2cBus&&) = delete;
    BaseI2cBus& operator=(BaseI2cBus&&) = delete;
     ...
     */
    [[nodiscard]] virtual hf_i2c_port_t GetPort() const noexcept { 
        return config_.port; 
    }
...
#endif // HAL_INTERNAL_INTERFACE_DRIVERS_BASEI2CBUS_H_
