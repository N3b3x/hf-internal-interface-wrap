...
/**
 * @brief Platform-agnostic CAN bus configuration (Classic CAN only for ESP32C6).
 */
struct CanBusConfig {
    ...
    CanBusConfig() noexcept 
        : tx_pin(-1), rx_pin(-1), baudrate(500000)
        , loopback_mode(false), silent_mode(false)
        , tx_queue_size(10), rx_queue_size(10) {}
};

/**
 * @brief Classic CAN message structure (ESP32C6 TWAI compatible)
 */
struct CanMessage {
