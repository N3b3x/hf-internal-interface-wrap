...
void temperature_reading_callback(const HfTempReading_t* reading, void* user_data) {
  ...
  if (reading->is_valid) {
    ...
    if (state->reading_count == 1) {
      ...
      if (temp < state->min_temperature)
        state->min_temperature = temp;
      if (temp > state->max_temperature)
        state->max_temperature = temp;

      // Running average
      state->avg_temperature =
          (state->avg_temperature * (state->reading_count - 1) + temp) / state->reading_count;
    }

    ESP_LOGI(TAG, "Continuous reading #%lu: %.2f°C (accuracy: ±%.1f°C)", state->reading_count, temp,
             reading->accuracy_celsius);
    ESP_LOGI(TAG, "Statistics - Min: %.2f°C, Max: %.2f°C, Avg: %.2f°C", state->min_temperature,
             state->max_temperature, state->avg_temperature);
  } else {
 ...
 */
void temperature_threshold_callback(float temperature_celsius, uint32_t threshold_type,
                                    void* user_data) {
  ExampleState* state = static_cast<ExampleState*>(user_data);
  ...
  ESP_LOGW(TAG, "THRESHOLD EXCEEDED! %s threshold reached: %.2f°C (count: %lu)", threshold_name,
           temperature_celsius, state->threshold_count);
}

/**
 * @brief Callback for temperature sensor errors
 */
void temperature_error_callback(HfTempError_t error, const char* error_description,
                                void* user_data) {
...
void example_sensor_info() {
  ...
  ESP_LOGI(TAG, "  Self-test: %s", temp_sensor->has_capability(TEMP_CAP_SELF_TEST) ? "Yes" : "No");
...
void example_esp32_ranges() {
  ...
  EspTempRange_t ranges[] = {ESP_TEMP_RANGE_NEG40_20, ESP_TEMP_RANGE_NEG30_50,
                             ESP_TEMP_RANGE_NEG10_80, ESP_TEMP_RANGE_20_100, ESP_TEMP_RANGE_50_125};

  const char* range_names[] = {"ESP_TEMP_RANGE_NEG40_20", "ESP_TEMP_RANGE_NEG30_50",
                               "ESP_TEMP_RANGE_NEG10_80", "ESP_TEMP_RANGE_20_100",
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (error == TEMP_SUCCESS) {
      ESP_LOGI(TAG, "  %s: %.0f°C to %.0f°C (±%.1f°C)", range_names[i], min_temp, max_temp,
               accuracy);
    }
  }
  ...
  float test_ranges[][2] = {{0.0f, 50.0f}, {-20.0f, 60.0f}, {25.0f, 75.0f}, {-35.0f, 40.0f}};
  ...
  for (int i = 0; i < 4; i++) {
    ...
    ESP_LOGI(TAG, "  Range [%.1f, %.1f]°C -> %d", test_ranges[i][0], test_ranges[i][1], optimal);
  }
}
...
void example_threshold_monitoring() {
  ...
  while ((esp_timer_get_time() - start_time) < duration_us) {
    ...
    if (error == TEMP_SUCCESS) {
      ESP_LOGI(TAG, "Temperature: %.2f°C (thresholds: %.1f - %.1f°C)", temp, low_threshold,
               high_threshold);
    }
...
void example_continuous_monitoring() {
  ...
  HfTempError_t error = temp_sensor->start_continuous_monitoring(
      sample_rate, temperature_reading_callback, &example_state);
  if (error != TEMP_SUCCESS) {
  ...
  ESP_LOGI(TAG, "  Temperature range: %.2f°C to %.2f°C", example_state.min_temperature,
           example_state.max_temperature);
