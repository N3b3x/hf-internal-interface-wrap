...
void test_bluetooth_basic_operations() {
    ...
    if (ret == hf_bluetooth_err_t::BLUETOOTH_SUCCESS) {
        ESP_LOGI(TAG, "✓ Local BLE address: %02X:%02X:%02X:%02X:%02X:%02X",
    ...
    if (ret == hf_bluetooth_err_t::BLUETOOTH_SUCCESS) {
        ...
        for (size_t i = 0; i < discovered_devices.size() && i < 10; i++) {
            ...
            ESP_LOGI(TAG, "    Address: %02X:%02X:%02X:%02X:%02X:%02X",
                     device.address.addr[0], device.address.addr[1], device.address.addr[2],
                     device.address.addr[3], device.address.addr[4], device.address.addr[5]);
