...
void ThreadSafeCanExample() {
    ...
    if (sf_can->Initialize()) {
        ...
        if (sf_can->Start()) {
            ...
            std::cout << "Status - Initialized: " << initialized 
                      << ", TX Full: " << tx_full 
...
void MultiThreadedCanExample() {
    ...
    std::thread receiver_thread([sf_can]() {
        for (int i = 0; i < 5; ++i) {
            ...
            if (sf_can->ReceiveMessage(msg, 2000)) {
                std::cout << "Receiver: Got message ID 0x" << std::hex << msg.id 
                          << ", DLC " << std::dec << static_cast<int>(msg.dlc) << std::endl;
            } else {
    ...
    std::cout << "  Contention rate: " << 
                 (100.0 * final_stats.lock_contentions / final_stats.total_operations) << "%" << std::endl;
