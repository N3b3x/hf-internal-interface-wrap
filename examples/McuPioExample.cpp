...
public:
    ...
    void SetPixelColor(uint8_t r, uint8_t g, uint8_t b) {
        ...
        if (result != HfPioErr::PIO_SUCCESS) {
            std::cerr << "Failed to transmit WS2812 data: " 
...
public:
    ...
    void SendNECCommand(uint8_t address, uint8_t command) {
        ...
        HfPioErr result = pio_.Transmit(channel_id_, symbols.data(), symbols.size(), true);
        if (result != HfPioErr::PIO_SUCCESS) {
            std::cerr << "Failed to transmit IR command: " 
                      << static_cast<int>(result) << std::endl;
        }
    }
...
public:
    CustomProtocolExample(uint8_t tx_ch, hf_gpio_num_t tx_pin, 
                          uint8_t rx_ch, hf_gpio_num_t rx_pin) 
        ...
        // Set up callbacks
        pio_.SetReceiveCallback([this](uint8_t channel, const PioSymbol* symbols, 
                                      size_t count, void* user_data) {
            OnDataReceived(channel, symbols, count);
        });
        
        pio_.SetErrorCallback([this](uint8_t channel, HfPioErr error, void* user_data) {
    ...
    void SendData(const std::vector<uint8_t>& data) {
        ...
        if (result != HfPioErr::PIO_SUCCESS) {
            std::cerr << "Failed to transmit custom protocol data: " 
                      << static_cast<int>(result) << std::endl;
        }
    }
    ...
    void StartReceiving() {
        ...
        HfPioErr result = pio_.StartReceive(rx_channel_, received_data_.data(), 
                                           received_data_.size(), 10000); // 10ms timeout
        if (result != HfPioErr::PIO_SUCCESS) {
            std::cerr << "Failed to start reception: " 
                      << static_cast<int>(result) << std::endl;
        }
    }
...
private:
    void OnDataReceived(uint8_t channel, const PioSymbol* symbols, size_t count) {
        std::cout << "Received " << count << " symbols on channel " 
