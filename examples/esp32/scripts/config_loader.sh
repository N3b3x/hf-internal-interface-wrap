...
# This script provides configuration functions for ESP32 applications
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]] && ([ "$1" = "--help" ] || [ "$1" = "-h" ]); then
  ...
  echo "  get_idf_version_smart     - Get smart default IDF version for app/build type"
  echo ""
  ...
  echo "  build_types=\$(get_build_types)"
  ...
  echo "  idf_version=\$(get_idf_version_smart \"gpio_test\" \"Release\")"
...
validate_app_build_type() {
    ...
    if [[ -n "$app_build_types" ]]; then
        ...
        for type in $clean_build_types; do
...
# Get version index for nested array access
get_version_index() {
    local target_version="$1"
    local versions_list="$2"
    ...
    while IFS= read -r version; do
        ...
        ((index++))
    done < <(echo "$versions_list" | tr ',' '\n')
    
    echo "0"  # Default to first index if not found
}
...
# Enhanced get_idf_version with build type matching
