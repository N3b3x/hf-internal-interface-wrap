...
CommBusTestManager::CommBusTestManager() {
  ...
  testManager_.SetAfterTestHook([this](bool result, CommBusTestType testType) {
    const char *testNames[] = {
        "I2C Initialization",      "I2C Configuration",    "I2C Basic Read/Write",
        "I2C Register Operations", "I2C Device Scan",      "I2C Error Handling",
        "I2C Thread Safety",       "SPI Initialization",   "SPI Configuration",
        "SPI Basic Transfer",      "SPI Full Duplex",      "SPI Chip Select",
        "SPI Error Handling",      "SPI Thread Safety",    "UART Initialization",
        "UART Configuration",      "UART Basic TX/RX",     "UART Formatted Output",
        "UART Flow Control",       "UART Error Handling",  "UART Thread Safety",
...
void CommBusTestManager::SetupI2cTests() {
  testManager_.AddTest(
      CommBusTestType::I2C_INITIALIZATION_TEST, [this]() { return TestI2cInitialization(); },
      false);
  testManager_.AddTest(
      CommBusTestType::I2C_CONFIGURATION_TEST, [this]() { return TestI2cConfiguration(); }, false);
  testManager_.AddTest(
      CommBusTestType::I2C_BASIC_READ_WRITE_TEST, [this]() { return TestI2cBasicReadWrite(); },
      true);
  testManager_.AddTest(
      CommBusTestType::I2C_REGISTER_OPERATIONS_TEST,
      [this]() { return TestI2cRegisterOperations(); }, true);
  testManager_.AddTest(
      CommBusTestType::I2C_DEVICE_SCAN_TEST, [this]() { return TestI2cDeviceScan(); }, true);
  testManager_.AddTest(
      CommBusTestType::I2C_ERROR_HANDLING_TEST, [this]() { return TestI2cErrorHandling(); }, true);
  testManager_.AddTest(
      CommBusTestType::I2C_THREAD_SAFETY_TEST, [this]() { return TestI2cThreadSafety(); }, true);
}

void CommBusTestManager::SetupSpiTests() {
  testManager_.AddTest(
      CommBusTestType::SPI_INITIALIZATION_TEST, [this]() { return TestSpiInitialization(); },
      false);
  testManager_.AddTest(
      CommBusTestType::SPI_CONFIGURATION_TEST, [this]() { return TestSpiConfiguration(); }, false);
  testManager_.AddTest(
      CommBusTestType::SPI_BASIC_TRANSFER_TEST, [this]() { return TestSpiBasicTransfer(); }, true);
  testManager_.AddTest(
      CommBusTestType::SPI_FULL_DUPLEX_TEST, [this]() { return TestSpiFullDuplex(); }, true);
  testManager_.AddTest(
      CommBusTestType::SPI_CHIP_SELECT_TEST, [this]() { return TestSpiChipSelect(); }, true);
  testManager_.AddTest(
      CommBusTestType::SPI_ERROR_HANDLING_TEST, [this]() { return TestSpiErrorHandling(); }, true);
  testManager_.AddTest(
      CommBusTestType::SPI_THREAD_SAFETY_TEST, [this]() { return TestSpiThreadSafety(); }, true);
}

void CommBusTestManager::SetupUartTests() {
  testManager_.AddTest(
      CommBusTestType::UART_INITIALIZATION_TEST, [this]() { return TestUartInitialization(); },
      false);
  testManager_.AddTest(
      CommBusTestType::UART_CONFIGURATION_TEST, [this]() { return TestUartConfiguration(); },
      false);
  testManager_.AddTest(
      CommBusTestType::UART_BASIC_TX_RX_TEST, [this]() { return TestUartBasicTxRx(); }, true);
  testManager_.AddTest(
      CommBusTestType::UART_FORMATTED_OUTPUT_TEST, [this]() { return TestUartFormattedOutput(); },
      true);
  testManager_.AddTest(
      CommBusTestType::UART_FLOW_CONTROL_TEST, [this]() { return TestUartFlowControl(); }, true);
  testManager_.AddTest(
      CommBusTestType::UART_ERROR_HANDLING_TEST, [this]() { return TestUartErrorHandling(); },
      true);
  testManager_.AddTest(
      CommBusTestType::UART_THREAD_SAFETY_TEST, [this]() { return TestUartThreadSafety(); }, true);
}

void CommBusTestManager::SetupIntegrationTests() {
  testManager_.AddTest(
      CommBusTestType::MULTI_BUS_COORDINATION_TEST, [this]() { return TestMultiBusCoordination(); },
      true);
  testManager_.AddTest(
      CommBusTestType::PERFORMANCE_BENCHMARK_TEST, [this]() { return TestPerformanceBenchmark(); },
      true);
}
...
bool CommBusTestManager::TestI2cConfiguration() {
  ...
  return (config.port == 0 && config.sda_pin == 21 && config.scl_pin == 22 &&
          config.clock_speed_hz == 100000);
}
  ...
  for (int i = 0; i < numThreads; ++i) {
    threads.emplace_back([this, &successCount, operationsPerThread]() {
      ...
      for (int j = 0; j < operationsPerThread; ++j) {
        ...
        if (result == HfI2cErr::I2C_SUCCESS || result == HfI2cErr::I2C_ERR_DEVICE_NOT_FOUND) {
          localSuccessCount++;
        }
...
bool CommBusTestManager::TestSpiConfiguration() {
  ...
  return (config.mosi_pin == 23 && config.miso_pin == 19 && config.sclk_pin == 18 &&
          config.cs_pin == 5);
}
...
bool CommBusTestManager::TestSpiBasicTransfer() {
  ...
  return (result == HfSpiErr::SPI_SUCCESS || result == HfSpiErr::SPI_ERR_DEVICE_NOT_RESPONDING);
}
...
bool CommBusTestManager::TestSpiThreadSafety() {
  ...
  for (int i = 0; i < numThreads; ++i) {
    threads.emplace_back([this, &successCount]() {
      ...
      if (result == HfSpiErr::SPI_SUCCESS || result == HfSpiErr::SPI_ERR_DEVICE_NOT_RESPONDING) {
        successCount++;
      }
    });
  }
...
bool CommBusTestManager::TestUartConfiguration() {
  ...
  return (config.uart_port == 2 && config.tx_pin == 17 && config.rx_pin == 16 &&
          config.baud_rate == 115200);
}
...
bool CommBusTestManager::TestUartBasicTxRx() {
  ...
  HfUartErr result =
      uartDriver_->Write(reinterpret_cast<const uint8_t *>(testString), strlen(testString));
  ...
  size_t bytesRead = 0;
  result = uartDriver_->Read(reinterpret_cast<uint8_t *>(rxBuffer), sizeof(rxBuffer) - 1, bytesRead,
                             100);

  // Should complete without error (may read 0 bytes)
  return (result == HfUartErr::UART_SUCCESS || result == HfUartErr::UART_ERR_TIMEOUT);
}
...
bool CommBusTestManager::TestUartThreadSafety() {
  ...
  for (int i = 0; i < numThreads; ++i) {
    threads.emplace_back([this, &successCount, i]() {
      ...
      HfUartErr result = uartDriver_->Write(reinterpret_cast<const uint8_t *>(testData.c_str()),
                                            testData.length());
      if (result == HfUartErr::UART_SUCCESS) {
        successCount++;
      }
...
bool CommBusTestManager::TestMultiBusCoordination() {
  ...
  threads.emplace_back([this, &successCount]() {
    const char *msg = "Test\n";
    HfUartErr result = uartDriver_->Write(reinterpret_cast<const uint8_t *>(msg), strlen(msg));
    if (result == HfUartErr::UART_SUCCESS) {
      successCount++;
    }
  });
...
bool CommBusTestManager::TestPerformanceBenchmark() {
  ...
  std::cout << "Performance test completed " << iterations << " iterations in " << duration.count()
            << "ms" << std::endl;
...
MockI2cDevice::MockI2cDevice(uint8_t address) : address_(address), respondToScan_(true) {
...
bool MockUartDevice::ProcessData(const uint8_t *txData, size_t txLength, uint8_t *rxData,
                                 size_t &rxLength) {
...
extern "C" int RunCommBusTests() {
  ...
  std::cout << "Communication Bus Tests " << (result ? "PASSED" : "FAILED") << std::endl;
