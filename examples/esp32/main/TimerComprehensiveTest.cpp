...
struct CallbackTestData {
    ...
    CallbackTestData() : call_count(0), last_call_time_us(0), min_interval_us(UINT64_MAX), 
                        max_interval_us(0), total_interval_us(0), callback_executed(false), 
...
bool test_timer_start_stop() noexcept {
    ...
    if (period_result != hf_timer_err_t::TIMER_SUCCESS || retrieved_period != test_period_us) {
        ESP_LOGE(TAG, "Timer period mismatch. Expected: %llu, Got: %llu", test_period_us, retrieved_period);
        return false;
    }
    ...
    if (g_callback_data.call_count < 2) {
        ESP_LOGE(TAG, "Expected at least 2 callbacks, got %lu", (unsigned long)g_callback_data.call_count);
        return false;
    }
    
    ESP_LOGI(TAG, "[PASS] Timer callbacks executed (%lu times)", (unsigned long)g_callback_data.call_count);
...
bool test_timer_period_validation() noexcept {
    ...
    ESP_LOGI(TAG, "Timer capabilities - Min: %llu us, Max: %llu us, Resolution: %llu us", 
             min_period, max_period, resolution);
...
bool test_timer_callbacks() noexcept {
    ...
    if (g_callback_data.call_count < 3) {
        ESP_LOGE(TAG, "Expected at least 3 callbacks, got %lu", (unsigned long)g_callback_data.call_count);
        return false;
    }
    ...
    if (g_callback_data.call_count > 1) {
        ...
        if (avg_interval < (expected_interval - tolerance) || avg_interval > (expected_interval + tolerance)) {
            ESP_LOGE(TAG, "Timing precision poor. Expected: %llu, Average: %llu", expected_interval, avg_interval);
            return false;
        }
...
bool test_timer_statistics() noexcept {
    ...
    if (callback_count < 3) {
        ESP_LOGE(TAG, "Statistics should show executed callbacks. Expected >= 3, got %llu", callback_count);
        return false;
    }
    ...
    if (detailed_result == hf_timer_err_t::TIMER_SUCCESS) {
        ...
        ESP_LOGI(TAG, "[INFO] Enhanced statistics not implemented (error: %d)", static_cast<int>(detailed_result));
    }
