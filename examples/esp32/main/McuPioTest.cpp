...
private:
  ...
  void TestChannelConfiguration() {
    ...
    assert(result == HfPioErr::PIO_SUCCESS || result == HfPioErr::PIO_ERR_UNSUPPORTED_OPERATION);
  ...
  void TestSymbolValidation() {
    ...
    if (result != HfPioErr::PIO_SUCCESS && result != HfPioErr::PIO_ERR_UNSUPPORTED_OPERATION) {
      std::cout << "✓ Symbol validation tests skipped (platform not supported)" << std::endl;
      return;
    }

    // Test valid symbols
    std::vector<PioSymbol> symbols = {{1000, true}, {500, false}, {2000, true}};
  ...
  void TestCapabilities() {
    ...
    std::cout << "✓ Resolution range: " << caps.min_resolution_ns << " - " << caps.max_resolution_ns
              << " ns" << std::endl;
