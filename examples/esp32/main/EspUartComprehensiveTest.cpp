/**
 * @file EspUartComprehensiveTest.cpp
...
static const char* TAG = "EspUART_Test";
...
bool test_uart_basic_write_read() noexcept;
...
bool test_uart_interrupt_operations() noexcept;
...
hf_uart_config_t create_test_config(hf_u8_t port) noexcept {
...
bool test_uart_basic_write_read() noexcept {
  ESP_LOGI(TAG, "Testing basic UART write/read operations...");
  ...
  ESP_LOGI(TAG, "[SUCCESS] Basic write/read operations completed");
  return true;
}
...
bool test_uart_interrupt_operations() noexcept {
  ESP_LOGI(TAG, "Testing UART interrupt operations...");
  ...
  ESP_LOGI(TAG, "[SUCCESS] Interrupt operations test completed");
  return true;
}
...
extern "C" void app_main(void) {
  ESP_LOGI(TAG, "╔══════════════════════════════════════════════════════════════════════════════╗");
  ESP_LOGI(TAG, "║                  ESP32-C6 ESPUART COMPREHENSIVE TEST SUITE                  ║");
  ...
  RUN_TEST(test_uart_basic_write_read);
  ...
  RUN_TEST(test_uart_interrupt_operations);
  ...
  print_test_summary(g_test_results, "ESPUART", TAG);

  if (g_test_results.failed_tests == 0) {
    ESP_LOGI(TAG, "[SUCCESS] ALL ESPUART TESTS PASSED!");
  } else {
