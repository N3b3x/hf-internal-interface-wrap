 ...
 * @brief Comprehensive example demonstrating all advanced features of the refactored EspPwm
 * implementation.
 *
 ...
 * @param user_data User data (unused)
 */
void IRAM_ATTR FaultCallback(hf_channel_id_t channel_id, hf_pwm_err_t error,
                             void* user_data) noexcept {
  ESP_LOGE(TAG, "PWM fault on channel %lu: %s", channel_id, hf_pwm_err_tToString(error));
...
void DemonstrateServoControl(EspPwm& pwm) {
  ...
  for (int i = 0; i < 3; i++) {
    ESP_LOGI(TAG, "Moving servo to %s position (%.1f%%)...", position_names[i],
             servo_positions[i] * 100.0f);
...
void DemonstrateAdvancedFeatures(EspPwm& pwm) {
  ...
  ESP_LOGI(TAG, "Current PWM mode: %s", (current_mode == hf_pwm_mode_t::Basic) ? "Basic" : "Fade");
