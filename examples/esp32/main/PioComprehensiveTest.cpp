 ...
 *
 * This comprehensive test suite validates all functionality of the EspPio class using ESP-IDF v5.5 RMT:
...
bool test_multiple_channel_configuration() noexcept {
    ...
    if (result1 != hf_pio_err_t::PIO_SUCCESS || result2 != hf_pio_err_t::PIO_SUCCESS) {
        ESP_LOGE(TAG, "Failed to configure multiple channels: TX=%s, RX=%s", 
                 HfPioErrToString(result1), HfPioErrToString(result2));
        return false;
    }
...
bool test_logic_analyzer_patterns() noexcept {
    ...
    ESP_LOGI(TAG, "Pattern: 1µs H, 1µs L, 2µs H, 2µs L, 0.5µs H, 0.5µs L, 3µs H, 1.5µs L, 0.75µs H, 4µs L");
    
    // Transmit pattern multiple times for easier capture
    for (int i = 0; i < 5; i++) {
        hf_pio_err_t result = pio.Transmit(0, test_symbols, symbol_count, true);
        if (result != hf_pio_err_t::PIO_SUCCESS) {
            ESP_LOGE(TAG, "Failed to transmit test pattern iteration %d: %s", 
                     i, HfPioErrToString(result));
            return false;
        }
...
bool test_frequency_sweep() noexcept {
    ...
    for (size_t f = 0; f < sizeof(frequencies) / sizeof(frequencies[0]); f++) {
        ...
        hf_pio_symbol_t square_wave[] = {
            {half_period_ticks, true},
            {half_period_ticks, false}
        };
        
        ESP_LOGI(TAG, "Generating %uHz square wave (%uns period)", 
                 frequencies[f], period_ns);
...
bool test_rmt_advanced_configuration() noexcept {
    ...
    if (result != hf_pio_err_t::PIO_SUCCESS) {
        ESP_LOGI(TAG, "Idle level configuration not supported: %s", 
                 HfPioErrToString(result));
    }
...
void test_receive_callback(hf_u8_t channel_id, const hf_pio_symbol_t* symbols, 
...
void test_error_callback(hf_u8_t channel_id, hf_pio_err_t error, void* user_data) {
    ESP_LOGI(TAG, "Error callback: channel %d, error %s", 
             channel_id, HfPioErrToString(error));
...
bool test_stress_transmission() noexcept {
    ...
    for (int i = 0; i < STRESS_ITERATIONS; i++) {
        ...
        if (result != hf_pio_err_t::PIO_SUCCESS) {
            ESP_LOGE(TAG, "Stress transmission failed on iteration %d: %s", 
                     i, HfPioErrToString(result));
            return false;
        }
    }
    ...
    ESP_LOGI(TAG, "Stress test completed: %d transmissions of %d symbols each", 
             STRESS_ITERATIONS, STRESS_SYMBOL_COUNT);
    ESP_LOGI(TAG, "Total time: %llu µs, Average per transmission: %llu µs", 
             total_time, total_time / STRESS_ITERATIONS);
