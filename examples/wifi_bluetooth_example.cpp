...
public:
  ...
  WiFiBluetoothDemo() : m_demo_running(false) {
    ...
    constexpr EspWifiAdvancedConfig wifi_config = {.enable_power_save = true,
    ...
    constexpr EspBluetoothAdvancedConfig bt_config = {.enable_power_save = true,
...
private:
  ...
  bool initializeBluetooth() {
    ...
    if (result != HfBluetoothErr::BLUETOOTH_SUCCESS) {
      ESP_LOGE(TAG_BT, "Bluetooth initialization failed: %s",
               BaseBluetooth::getErrorString(result));
      return false;
    }
  ...
  void registerEventHandlers() {
    // WiFi event handler
    m_wifi->registerEventCallback(
        [this](HfWifiEvent event, void* event_data) { handleWifiEvent(event, event_data); });
    ...
    m_bluetooth->registerDataCallback(
        [this](const HfBluetoothAddress& address, const std::vector<uint8_t>& data) {
    ...
    if (result == HfWifiErr::WIFI_SUCCESS && !networks.empty()) {
      ...
      for (const auto& network : networks) {
        ESP_LOGI(TAG_WIFI, "  SSID: %s, RSSI: %d dBm, Channel: %d, Security: %d",
                 network.ssid.c_str(), static_cast<int>(network.rssi),
                 static_cast<int>(network.channel), static_cast<int>(network.security));
      }
    } else {
  ...
  void demonstrateBluetoothAdvertising() {
    ...
    if (result == HfBluetoothErr::BLUETOOTH_SUCCESS) {
      ...
      ESP_LOGE(TAG_BT, "Failed to start BLE advertising: %s",
               BaseBluetooth::getErrorString(result));
    }
  }
  ...
  void demonstrateBluetoothScanning() {
    ...
    if (result == HfBluetoothErr::BLUETOOTH_SUCCESS && !devices.empty()) {
      ...
      for (const auto& device : devices) {
        ESP_LOGI(TAG_BT, "  Name: %s, Address: %s, RSSI: %d dBm, Type: %d", device.name.c_str(),
                 device.address.toString().c_str(), static_cast<int>(device.rssi),
  ...
  void demonstrateBluetoothGattServer() {
    ...
    uint16_t service_handle =
        esp_bluetooth->createGattService(Example::CUSTOM_SERVICE_UUID, true, 10);
    if (service_handle == 0) {
    ...
    uint16_t char_handle = esp_bluetooth->addGattCharacteristic(
        service_handle, Example::DATA_CHARACTERISTIC_UUID,
  ...
  void printStatus() {
    ...
    ESP_LOGI(TAG_WIFI, "WiFi State: %d, Connected: %s, AP Active: %s",
             static_cast<int>(m_wifi->getState()), m_wifi->isConnected() ? "Yes" : "No",
             m_wifi->isAccessPointActive() ? "Yes" : "No");

    if (m_wifi->isConnected()) {
      ESP_LOGI(TAG_WIFI, "RSSI: %d dBm, SSID: %s", static_cast<int>(m_wifi->getRssi()),
    ...
    ESP_LOGI(TAG_BT, "BT State: %d, Enabled: %s, Advertising: %s, Scanning: %s, Discoverable: %s",
             static_cast<int>(m_bluetooth->getState()), m_bluetooth->isEnabled() ? "Yes" : "No",
             m_bluetooth->isAdvertising() ? "Yes" : "No", m_bluetooth->isScanning() ? "Yes" : "No",
