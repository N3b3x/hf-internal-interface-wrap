 ...
 * logger.LogBufferChar("TEXT", (const char*)binary_data, sizeof(binary_data),
 * hf_log_level_t::LOG_LEVEL_INFO);
 * @endcode
 *
 * @note EspLogger instances cannot be copied or moved due to hardware resource management.
 * @note If you need to transfer ownership, use std::unique_ptr<EspLogger> or similar smart
 * pointers.
...
public:
   ...
   * @return hf_logger_err_t Success or error code
   */
  hf_logger_err_t Log(hf_log_level_t level, const char* tag, const char* format,
                      ...) noexcept override;
   ...
   * @return hf_logger_err_t Success or error code
   */
  hf_logger_err_t LogV(hf_log_level_t level, const char* tag, const char* format,
...
private:
   ...
   * @return hf_logger_err_t Success or error code
   */
  hf_logger_err_t FormatMessage(hf_log_level_t level, const char* tag, const char* file,
                                hf_u32_t line, const char* function, const char* format,
   ...
   * @return hf_logger_err_t Success or error code
   */
  hf_logger_err_t WriteMessage(hf_log_level_t level, const char* tag, const char* message,
   ...
   * @return hf_logger_err_t Success or error code
   */
  hf_logger_err_t WriteMessageV(hf_log_level_t level, const char* tag, const char* format,
