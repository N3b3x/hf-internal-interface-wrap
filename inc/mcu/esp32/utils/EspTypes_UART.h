...
struct hf_uart_config_t {
  ...
  hf_uart_config_t() noexcept
      : port_number(0), baud_rate(115200), data_bits(hf_uart_data_bits_t::HF_UART_DATA_8_BITS),
        parity(hf_uart_parity_t::HF_UART_PARITY_DISABLE),
        stop_bits(hf_uart_stop_bits_t::HF_UART_STOP_BITS_1),
        flow_control(hf_uart_flow_ctrl_t::HF_UART_HW_FLOWCTRL_DISABLE), tx_pin(HF_UART_IO_UNUSED),
        rx_pin(HF_UART_IO_UNUSED), rts_pin(HF_UART_IO_UNUSED), cts_pin(HF_UART_IO_UNUSED),
        tx_buffer_size(1024), rx_buffer_size(1024), event_queue_size(10),
        operating_mode(hf_uart_operating_mode_t::HF_UART_MODE_POLLING), timeout_ms(1000),
        enable_pattern_detection(false), enable_wakeup(false), enable_loopback(false) {}
};
...
struct hf_uart_flow_config_t {
  ...
  hf_uart_flow_config_t() noexcept
      : enable_hw_flow_control(false), enable_sw_flow_control(false), xon_char(0x11),
        xoff_char(0x13), rx_flow_ctrl_thresh(122), tx_flow_ctrl_thresh(80), auto_rts(false),
        auto_cts(false) {}
};
...
struct hf_uart_power_config_t {
  ...
  hf_uart_power_config_t() noexcept
      : sleep_retention_enable(true), allow_pd_in_light_sleep(true), allow_pd_in_deep_sleep(false),
        wakeup_enable(false), wakeup_threshold(3), wakeup_timeout_ms(1000) {}
};
...
struct hf_uart_pattern_config_t {
  ...
  hf_uart_pattern_config_t() noexcept
      : enable_pattern_detection(false), pattern_char('+'), pattern_char_num(1), char_timeout(5),
        post_idle(5), pre_idle(5) {}
};
...
struct hf_uart_rs485_config_t {
  ...
  hf_uart_rs485_config_t() noexcept
      : mode(hf_uart_mode_t::HF_UART_MODE_UART), enable_collision_detect(false),
        enable_echo_suppression(false), auto_rts_control(false), rts_delay_ms(0), rts_timeout_ms(0),
        collision_timeout_ms(0) {}
};
