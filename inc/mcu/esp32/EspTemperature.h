 ...
 */
using esp_temp_threshold_callback_t = std::function<void(EspTemperature* sensor, float temperature, bool is_high_threshold)>;

/**
 * @brief ESP32-C6 continuous monitoring callback function type
 */
using esp_temp_monitoring_callback_t = std::function<void(EspTemperature* sensor, float temperature, hf_u64_t timestamp_us)>;
...
public:
     ...
     * @return Error code (TEMP_SUCCESS on success)
     */
    hf_temp_err_t GetRangeInfo(esp_temp_range_t range, float* min_celsius, 
     ...
     */
    [[nodiscard]] esp_temp_range_t FindOptimalRange(float min_celsius, float max_celsius) const noexcept;
...
private:
     ...
     * @param accuracy_celsius Pointer to store accuracy
     */
    void GetRangeConfig(esp_temp_range_t range, float* min_celsius, 
                        float* max_celsius, float* accuracy_celsius) const noexcept;
