...
enum class hf_logger_err_t : hf_u32_t {
    HF_LOGGER_ERR_LIST(HF_LOGGER_ERR_ENUM)
};
...
public:
     ...
     * @return hf_logger_err_t Success or error code
     */
    virtual hf_logger_err_t LogWithLocation(hf_log_level_t level, const char* tag, 
...
#define HF_LOG_ERROR(tag, format, ...) \
    LogWithLocation(hf_log_level_t::LOG_LEVEL_ERROR, tag, __FILE__, __LINE__, __FUNCTION__, format, ##__VA_ARGS__)

/**
 * @brief Log at WARN level with file and line information
 */
#define HF_LOG_WARN(tag, format, ...) \
    LogWithLocation(hf_log_level_t::LOG_LEVEL_WARN, tag, __FILE__, __LINE__, __FUNCTION__, format, ##__VA_ARGS__)

/**
 * @brief Log at INFO level with file and line information
 */
#define HF_LOG_INFO(tag, format, ...) \
    LogWithLocation(hf_log_level_t::LOG_LEVEL_INFO, tag, __FILE__, __LINE__, __FUNCTION__, format, ##__VA_ARGS__)

/**
 * @brief Log at DEBUG level with file and line information
 */
#define HF_LOG_DEBUG(tag, format, ...) \
    LogWithLocation(hf_log_level_t::LOG_LEVEL_DEBUG, tag, __FILE__, __LINE__, __FUNCTION__, format, ##__VA_ARGS__)

/**
 * @brief Log at VERBOSE level with file and line information
 */
#define HF_LOG_VERBOSE(tag, format, ...) \
    LogWithLocation(hf_log_level_t::LOG_LEVEL_VERBOSE, tag, __FILE__, __LINE__, __FUNCTION__, format, ##__VA_ARGS__)
